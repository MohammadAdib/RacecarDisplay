...
public class OBDUtils {
    ...
    public static int getRPM(String rpm) {
        rpm = rpm.substring(rpm.length() - 5);
    ...
    public static int getLoad(String load) {
        return (int) ((Integer.decode("0x" + load.substring(load.length() - 3)) / 2.55) + 0.5);
    }
    ...
    private static int getTemp(String temp) {
        return Integer.decode("0x" + temp.substring(temp.length() - 3)) - 40;
    }
}
